...
def identify_unique_locations(df):
    ...
    output.write(f"Found {len(unique_cafes)} unique cafes:\n")
    for i, cafe in enumerate(unique_cafes):
        count = cafe_counts[cafe]
        output.write(f"{i+1}. {cafe} ({count} entries)\n")
    
    output.write(f"\nFound {len(unique_vendors)} unique vendors:\n")
    for i, vendor in enumerate(unique_vendors):
        count = vendor_counts[vendor]
        output.write(f"{i+1}. {vendor} ({count} entries)\n")
    
    return unique_cafes, unique_vendors, output.getvalue()
...
def generate_summary(analyzed_df):
    ...
    if len(analyzed_only) == 0:
        output.write("No entries were successfully analyzed.")
        return output.getvalue()
    
    output.write("\n=== ANALYSIS SUMMARY ===\n")
    output.write(f"Total entries analyzed: {len(analyzed_only)}\n")
    ...
    output.write(str(compliance_counts) + "\n")
    
    output.write("\nOverall Severity Levels:\n")
    output.write(str(severity_counts) + "\n")
...
def main(df):
    ...
    unique_cafes, unique_vendors, output_str = identify_unique_locations(df)
    ...
    # Run analysis
    analyzed_df = analyze_selected_locations(df, selected_cafes, selected_vendors)
